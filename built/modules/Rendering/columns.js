...
function drawColumnsAndGradients() {
    for (let i = 0; i < Init.columns.length; i++) {
        ...
        Init.context.fillRect(Init.columns[i].xPosition, 0, Init.columnWidth - 1, Init.noteScrollWindowHeight);
        // red timing bar line
        Init.context.fillStyle = "rgba(128, 0, 0, 1)";
        Init.context.fillRect(Init.columns[i].xPosition, Init.noteScrollWindowHeight - Init.columnWidth / 25, Init.columnWidth - 1, Init.columnWidth / 25);
        // draw full gradient timing boxes
        // grdOpacity = speed of gradient transparency
        let grdOpacity = 1 - columnFadeoutProgress[i] * 0.05;
        let gradientColor = columnFadeoutProgress[i] < Init.columnWidth / 2 ?
...
function drawGradientForNoteScrollWindowKeyHold(timingBoxIndex, gradientColor1, gradientColor2) {
    // partial shading
    let grd = Init.context.createLinearGradient(0, 0, 0, Init.canvas.height + 2 * (Init.columnWidth - 1));
    ...
    Init.context.fillRect(Init.columns[timingBoxIndex].xPosition +
        0.5 * columnFadeoutProgress[timingBoxIndex], 0, Init.columnWidth - 1 * columnFadeoutProgress[timingBoxIndex] - 1, Init.noteScrollWindowHeight);
}
export { drawColumnsAndGradients, drawGradientForNoteScrollWindowKeyHold };
