...
const activeHeldNotesHit = new Set();
function startHeldNotePulse(columnIndex) {
...
function stopHeldNotePulse(columnIndex) {
    HeldNotePulseInfo[columnIndex].width = 0;
    HeldNotePulseInfo[columnIndex].height = 0;
...
function drawHeldNodePulse() {
    ...
    activeHeldNotesHit.forEach(function (note) {
        ...
        if (GameState.columns[i].holdingDownNote) {
            ...
            while (j < GameState.heldNotesHit.length) {
                if (GameState.heldNotesHit[j].column === i) {
                    ...
                    // set gradient
                    let grdLeft = Init.context.createLinearGradient(Init.columns[i].xPosition, 0, Init.columns[i].xPosition + Init.columnWidth, 0);
                    ...
                    Init.context.fillRect(Init.columns[i].xPosition + (Init.columnWidth - 1) / 2 - HeldNotePulseInfo[i].width, Init.noteScrollWindowHeight - HeldNotePulseInfo[i].height, HeldNotePulseInfo[i].width, HeldNotePulseInfo[i].height);
                    // set gradient
                    let grdRight = Init.context.createLinearGradient(Init.columns[i].xPosition, 0, Init.columns[i].xPosition + Init.columnWidth, 0);
                    ...
                    Init.context.fillRect(Init.columns[i].xPosition + (Init.columnWidth - 1) / 2, Init.noteScrollWindowHeight - HeldNotePulseInfo[i].height, HeldNotePulseInfo[i].width, HeldNotePulseInfo[i].height);
                }
                j++;
            }
...
export { drawHeldNodePulse, startHeldNotePulse, stopHeldNotePulse };
