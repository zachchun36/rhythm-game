...
function drawYellowGlow(opacity) {
    let grd = Init.context.createLinearGradient(Init.columns[0].xPosition, -200, // arbitrary gradient
    Init.columns[0].xPosition, 1.3 * Init.noteScrollWindowHeight);
    grd.addColorStop(0, "rgba(0, 0, 0, 0)");
    grd.addColorStop(1, "rgba(214, 185, 11, " + opacity);
    Init.context.fillStyle = grd;
    Init.context.fillRect(Init.columns[0].xPosition, 0, Init.columnWidth * Init.columns.length - 1, Init.noteScrollWindowHeight);
}
...
export { drawFlairGlowFade, drawSmoothieTimeGlow };
