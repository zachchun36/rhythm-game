    ...
    if (event.repeat) {
        ...
        while (i < GameState.notes.length) {
            ...
            if (currentNote.column === index && !NoteTypes.isCompletedNote(currentNote)) {
                let noteHitByKeydown = processNoteHit(currentTime, currentNote, i);
                if (noteHitByKeydown) {
                    break;
                }
            }
...
function processNoteHit(currentTime, currentNote, i) {
    ...
    if (noteTiming) {
        mostRecentNoteIndex = i;
        let completedNote = NoteTypes.completeNote(currentNote, noteTiming);
        GameState.notes[i] = completedNote;
        Render.drawNoteTimingEffects(noteTiming, currentNote.time, GameState.notes[i].column);
        return true;
    }
    return false;
}
